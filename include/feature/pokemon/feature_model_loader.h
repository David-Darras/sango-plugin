...
public:
  ...
  static bool LoadPokemon(LoadedModel* out, Species species, Form form,
    ...
    const u32 pack_top = table.GetPackTop(static_cast<u16>(species),
                                          static_cast<u8>(form),
                                          static_cast<u8>(gender));
    if (pack_top == PokeModelTable::kInvalidPack) {
      ui::LogApplication::Print(u"unknown species %d",
                               static_cast<u16>(species));
      return false;
    }
