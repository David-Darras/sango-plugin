...
public:
  ...
  STATIC_INLINE void Initialize() {
    ...
    // phone numbers, etc.
    WRITE32(ADDRESS_KEYBOARD_WORD_FILTER, 0xE3A00000);
    ARM_RET(ADDRESS_KEYBOARD_WORD_FILTER_RETURN);
  }

  STATIC_INLINE void PatchLoad() {
    MEMORY_SCOPE(ADDRESS_MEMORY_REGION_KEYBOARD, 0x7000);
    HookManager::ForceEnable(HookID::kKeyboardUpdateKeys);
    // Force to refresh when pressing L
    ARM_NOP(ADDRESS_KEYBOARD_REFRESH_ON_L);
    // Pressing R is like pressing L
    WRITE32(ADDRESS_KEYBOARD_KEY_HANDLER_TABLE + 6 * 4,
            READ32(ADDRESS_KEYBOARD_KEY_HANDLER_TABLE + 4 * 4));
    // Don't switch between the two keyboard mode
    ARM_NOP(ADDRESS_KEYBOARD_MODE_SWITCH);

    // No animation
    ARM_NOP(ADDRESS_KEYBOARD_ANIMATION_1);
    ARM_NOP(ADDRESS_KEYBOARD_ANIMATION_2);
    ARM_NOP(ADDRESS_KEYBOARD_ANIMATION_3);
    ARM_NOP(ADDRESS_KEYBOARD_ANIMATION_4);
...
public:
  static void UpdateKeys(uptr self, u32 layout_id, bool is_qwerty,
    ...
    auto calculate_c = reinterpret_cast<u32(*)(u32, u32)>(ADDRESS_KEYBOARD_CALCULATE_KEY_INDEX);
