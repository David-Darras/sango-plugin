...
static void prev_##name(pokemon::CoreData& core, u32 value) {\
...
static void next_##name(pokemon::CoreData& core, u32 value) {\
auto& ctx = ui::AppStatus::GetInstance();\
...
static void print_##name(Pane* pane, AppLayoutManager& manager, pokemon::DataAccessor& accessor) {\
auto& ctx = ui::AppStatus::GetInstance();\
...
public:
  ...
  void Print(AppLayoutManager& manager, pokemon::DataAccessor& accessor,
             Color8& top,
             Color8& bottom) {
  ...
  void SetStringValue0(AppLayoutManager& manager, const c16* str, ...) const {
    ...
    ((void (*)(c16*, u32, const c16*, va_list))sys::address::kStdVswprintf)(
        String::GetTmpBuf(), sys::address::kBufferSize, str, args);
    va_end(args);
  ...
  void Prev(pokemon::CoreData& core, u32 value) { if (prev_) prev_(core, value); }
  void Next(pokemon::CoreData& core, u32 value) { if (next_) next_(core, value); }
...
static void toggle_shiny(pokemon::CoreData& core, u32 value) {
...
static Pane pane_shiny
    (true, 66, 68, 11, toggle_shiny, toggle_shiny,
     [](Pane* pane, AppLayoutManager& manager,
       ...
       auto& core = *accessor.GetCoreData();
       u32 is_shiny = pokemon::Utils::IsShiny(
           core.id, core.shiny_id);
       pane->SetStringValue1(manager, is_shiny ? u"Yes" : u"No");
     });

static Pane pane_level
    (true, 4, 6, 0xFF, [](pokemon::CoreData& core, u32 value) {
       s8 level = pokemon::Utils::GetLevelFromExperience(
           core.species, core.form, core.experience);
       level -= value;
       if (level < 1) level = 100;
       core.experience = pokemon::Utils::GetExperienceFromLevel(
    ...
    (true, 38, 40, 0xFF,
     [](pokemon::CoreData& core, u32 value) {
       if (AppStatus::GetInstance().is_restricted) {
         ...
         PREV(core.ability, value, static_cast<u8>(AbilityId::kCount));
       }
     }, [](pokemon::CoreData& core, u32 value) {
       if (AppStatus::GetInstance().is_restricted) {
         auto& pkm = global_data::Pokemon::GetInstance(core.species, core.form);
...
void AppStatus::Update(savedata::PokemonParam& pokemon,
  ...
  switch (mode_) {
    ...
    case Mode::kEditing: {
      refresh = true;
      accessor.Decrypt();
      pokemon::CoreData& core = *accessor.GetCoreData();
      switch (key) {
        case Key::kX:
          core.SetMaxContest();
          core.SetMaxIVs();
  ...
  if (refresh && key != Key::kNone) {
    ...
    ((void(*)(void*, u32, bool))pokemon::address::kAppStatusUpdatePokemon)(process,
      GetSlot(), false);
    if (!new_model) {
      WRITEB(
          READ32((uptr)process + kOffsetToRenderContext) +
...
void AppStatus::MoveAcrossPanes(sys::Controller& controller) {
  struct Direction {
    Pane* center;
    Pane* up;
    Pane* right;
