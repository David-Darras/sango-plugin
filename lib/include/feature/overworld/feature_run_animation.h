...
public:
  ...
  STATIC_INLINE void Initialize() {
    HookManager::Initialize(HookID::kModelPlayAnimation,
                            ADDRESS_MODEL_PLAY_ANIMATION, (uptr)PlayAnimationHook);
    HookManager::Initialize(HookID::kModelUpdateMotion,
                            ADDRESS_MODEL_UPDATE_MOTION, (uptr)UpdateMotionHook,
                            false);
  }

  STATIC_INLINE void PatchLoad() {
    HookManager::ForceEnable(HookID::kModelUpdateMotion);
  }
...
private:
  ...
  static u32 PlayAnimationHook(void* model, u32 action, u32 direction) {
    ...
    return HookManager::Call<u32>(HookID::kModelPlayAnimation, model, action,
                                  direction);
  }
  ...
  static u32 UpdateMotionHook(void* model, bool flag) {
    ...
    return HookManager::Call<u32>(HookID::kModelUpdateMotion, model, flag);
  }
