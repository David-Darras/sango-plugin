...
public:
  ...
  static bool OnLoad(void* self, const void*& buffer, u32& size) {
    ...
    ScriptDescriptor* descriptor = vm->GetDescriptor();
    ...
    buffer = ctx.stub_;
    size = ((const AmxHeader*)ctx.stub_)->total_file_size;
    return true;
  }
...
private:
  ...
  bool IsSuspendedElsewhere(const Engine* engine,
                            const ScriptVm* vm) const {
  ...
  static PawnCell NativeEntry(AmxRuntime* amx, const PawnCell*) {
    ...
    if (ScriptVm::FromAmx(amx) != ctx.vm_) return 0;
  ...
  void BuildStub() {
    auto* header = (AmxHeader*)stub_;
    auto* native = (AmxSymbol*)(stub_ + sizeof(AmxHeader));
    u32 offset = sizeof(AmxHeader) + sizeof(AmxSymbol);
    ...
    native->name_hash = CalcNameHash("CppScript");
    name_table[0] = 63;
    name_table[1] = 0;
    ...
    header->public_functions_offset = sizeof(AmxHeader);
    header->native_functions_offset = sizeof(AmxHeader);
    header->libraries_offset = sizeof(AmxHeader) +
                               sizeof(AmxSymbol);
  ...
  Context context_{natives_, coroutine_, message_buffer_,
                           kMessageCapacity};
};
