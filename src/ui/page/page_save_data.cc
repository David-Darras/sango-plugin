...
void LoadSaveDataPokemonPage(MainApplication& app, void* args) {
  ...
  if ((pkm->iv_flags >> 30) & 1) {
    app.Add("Remaining Steps Before Hatch", pkm->remaining_steps_before_hatch);
  }

  app.AddSeparator();

  app.AddMove("Move 1", pkm->moves[0])
     .AddMove("Move 2", pkm->moves[1])
     .AddMove("Move 3", pkm->moves[2])
     .AddMove("Move 4", pkm->moves[3])
     .AddSeparator();

  app.Add("PP 1", pkm->pp[0])
     .Add("PP 2", pkm->pp[1])
     .Add("PP 3", pkm->pp[2])
     .Add("PP 4", pkm->pp[3])
     .AddSeparator();
  ...
  app.Add("Contest - Cool", pkm->contest.cool)
...
void LoadSaveDataBagItemsPage(MainApplication& app, void* args) {
  ...
  app.Add("Pocket Select", pocket_id)
  ...
  app.Add("Slot Select", slot_idx)
     .WithBounds(0, max_slots - 1)
     .WithRefresh()
     .AddSeparator();

  app.AddItem("Item", target_pocket[slot_idx].id);

  app.Add("Quantity", target_pocket[slot_idx].count)
...
void LoadSaveDataRecordsPage(MainApplication& app, void* args) {
  ...
  app.Add("Is Disabled", data.is_disabled)
     .AddSeparator()
...
void LoadSaveDataOverworldMenuPage(MainApplication& app, void* args) {
  ...
  app.Add("Is Pokemon List Visible", &data.flags, 0, 1)
...
void LoadSaveDataMinigamePage(MainApplication& app, void* args) {
  ...
  app.Add("Difficulty Choice", choice)
     .WithArray(DIFFICULTIES, 4)
     .WithRefresh();

  app.AddSeparator();

  app.Add("Berry Picker Score", data.berry_picker_high_scores[choice])
     .WithBounds(0, 999)
     .Add("Berry Picker Rating", data.berry_picker_best_ratings[choice])
     .WithArray(RATINGS, SIZE(RATINGS))
     .AddSeparator();

  app.Add("Head It Score", data.head_it_high_scores[choice])
     .WithBounds(0, 9999)
     .Add("Head It Rating", data.head_it_best_ratings[choice])
     .WithArray(RATINGS, SIZE(RATINGS))
     .AddSeparator();

  app.Add("Tile Puzzle Index", puzzle_idx)
     .WithBounds(0, 4)
     .WithRefresh();

  auto& puzzle = data.tile_puzzle_scores[choice];

  app.Add("Puzzle Score", puzzle.total_score[puzzle_idx])
...
void LoadSaveDataPokemonAmiePage(MainApplication& app, void* args) {
  ...
  app.Add("Select Slot", puff_idx)
     .WithBounds(0, savedata::PokemonAmie::kMaxPokePuffs - 1)
     .WithRefresh();

  app.Add("Poké Puff ID", data.poke_puffs[puff_idx])
     .WithBounds(0, savedata::PokemonAmie::kMaxPokePuffId);

  app.Add("Last Opened (Days)", data.last_opened_timestamp);
}
...
void LoadSaveDataBagMetadataPage(MainApplication& app, void* args) {
  ...
  app.Add("Pocket Select", pocket_idx)
...
void LoadSaveDataPokedexPage(MainApplication& app, void* args) {
  ...
  app.Add("Spinda Pattern", data.spinda_pattern)
     .AddSeparator();
...
void LoadSaveDataOPowerPage(MainApplication& app, void* args) {
  ...
  app.Add("OPower Points", man.power_points)
...
void LoadSaveDataSettingsPage(MainApplication& app, void* args) {
  ...
  app.Add("Text Speed", &settings.core, 0, 2)
...
void LoadSaveDataPssGroupPage(MainApplication& app, void* args) {
  ...
  app.Add("Profile Index", choice).AddSeparator();
